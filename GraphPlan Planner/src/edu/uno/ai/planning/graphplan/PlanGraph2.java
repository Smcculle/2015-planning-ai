...
public class PlanGraph2
{
	...
	ArrayList<PlanGraphLiteral> goals;
	...
	static public PlanGraph2 create(Problem problem, boolean calculateMutex)
	{
		PlanGraph2 current = new PlanGraph2(problem, calculateMutex);
		Expression goal = problem.goal;
		
		
		while (!(current.containsGoal(goal) && current.isGoalNonMutex(goal)) && !current.isLeveledOff())
			current = new PlanGraph2(current);
		
		return current;
	}
	
	static public PlanGraph2 create(Problem problem)
	{
		return PlanGraph2.create(problem, true);
	}
	 ...
	 */
	public PlanGraph2 (Problem problem, boolean calculateMutex)
	{
		_parent = null;
		...
		goals = new ArrayList<PlanGraphLiteral>();
		ArrayList<Literal> tmp =  expressionToLiterals(problem.goal);
		for (Literal l : tmp){
			goals.add(new PlanGraphLiteral(l));
		}
	 ...
	 */
	public PlanGraph2 (Problem problem)
	{
	 ...
	 */
	public PlanGraph2 (PlanGraph2 parent)
	{
		_parent = parent;
